...
class Win(GlutWindow):
    ...
    def draw(self):
        ...
        mvp_stack[-1] = glm.rotate(mvp_stack[-1], time.time(), glm.vec3(0, 1, 0)) # Added
        self.model_matrix = mvp_stack[-1]
        self.model_matrix = glm.rotate(self.model_matrix, time.time(), glm.vec3(0, 1, 0)) # Added
        ...
        mvp_stack[-1] = glm.rotate(mvp_stack[-1], 1.2* time.time(), glm.vec3(0, 1, 0)) # Added
        self.model_matrix = mvp_stack[-1]
        self.model_matrix = glm.rotate(self.model_matrix, time.time(), glm.vec3(0, 1, 0)) # Added
        self.calc_mvp()
        glUniformMatrix4fv(self.context.mvp_location, 1, GL_FALSE, glm.value_ptr(self.context.mvp))
